...
class tapPage extends StatefulWidget {
  ...
  const tapPage({super.key, required this.esewaName});
...
class _tapPageState extends State<tapPage> {
  ...
  double _balance = 0.0;
  void updateNumberValue(double value) {
    setState(() {
      // balance = value;
    });
  }
  ...
  void showSuccessDialog() {
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text('Amount Loaded Successfully'),
          actions: [
            TextButton(
              onPressed: () {
